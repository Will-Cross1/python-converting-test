...
"""
Concatenation of E-PROFILE data to daily files of pre-existing daily files in archive in CEDA

This builds on the previous concat_for_ingest as it needs to work steadily though the existing files
and remove files that have been successfully concatenated from the archive as a clean-up step at the end.

Rolf Ruefenacht, Meteoswiss, 02/2020


Operationalised to run on CEDA systems:
Graham Parton, CEDA, 03/2022
"""
...
PROCESSING_DIR = '/gws/nopw/j04/cedaproc/eprofile_for_ingest/eprofile/quarantine'
ARRIVALS_DIR = '/gws/nopw/j04/cedaproc/eprofile_for_ingest/eprofile/arrivals'
...
def concat_single_inst(pattern_in, filename_out, delete_after_concat=False, ignore_previous_concat=False,
    ...
    if os.path.exists(filename_out):
        ...
        readyToIngest_path = filename_out.replace('quarantine', 'readyToIngest')            #starts here
    ...
    try:
        log.info(f'now moving to do concat file.... of {len(pattern_in)} new files')            #next goes here
        if pattern_in:

            with xr.open_mfdataset(pattern_in, concat_dim="time", data_vars='minimal',
                                   coords='minimal',
                                   compat='override') as ds:  # for working with xr version 0.10.2 installed on JASMIN
                ...
                # resolve mislabel of Conventions global attribute:
                if  'Convention' in ds2.attrs:
                    ds2.attrs['Conventions'] = ds2.attrs.pop('Convention')
                ...
                if contrib:
                    file_comments += f"{lead_file_comments} {'|'.join(contrib)}"
                ...
                if not 'comments' in ds2.quality_flag.attrs:
                    ds2.quality_flag.attrs['comments'] = f"""flag_values: 0,1,2.  flag_meanings: 0: valid data;  1: do_not_use; 2: no_information.
The invalid flag (=1) is attributed to all data >1000m above cloud base, the other points have a valid flag (=0)"""
                
                else:
                    ds2.quality_flag.attrs['comments'] = f"{ds2.quality_flag.attrs['comments']}.\nThe invalid flag (=1) is attributed to all data >1000m above cloud base, the other points have a valid flag (=0)"
                ...
                ds2.latitude.attrs['units'] = 'degree_north'
                ds2.longitude.attrs['units'] = 'degree_east'

                ds2.cloud_amount.attrs['long_name'] = 'Cloud amount in octa'
                ds2.cloud_amount.attrs['units'] = '1'
            ...
            shutil.copy2(temp_name, filename_out)
        ...
        log.error(f"{e}: {[pattern_in]}")       #ends up here
        return
    except:
        raise
    else:
...
def moveToIngest(file_to_ingest):

    dst = file_to_ingest.replace(PROCESSING_DIR,ARRIVALS_DIR)
    
    if os.path.exists(file_to_ingest):
        os.chmod(file_to_ingest,0o660)
        os.rename(file_to_ingest,dst)
...
def main(start_dir, verbose = 2):
    ...
    files_to_concat = glob.glob(os.path.join(start_dir,'L2*.nc'))
    log.debug(files_to_concat)
    ...
    if files_to_concat:
        ...
        file_out = re.search('(?P<outfilename>L2_[\w`-]+_[\w]{1}[0-9]{8})',files_to_concat[0]).groupdict()['outfilename'] + '.nc'

        filename_out = os.path.join(PROCESSING_DIR, file_out)
        log.info(f'{filename_out}, {files_to_concat}')
        
        

        file_to_ingest = concat_single_inst(files_to_concat, filename_out, delete_after_concat=False,
                               ignore_previous_concat=False, time_as_limited_dim=True)

#file_to_ingest sometimes is returned as a None type (something to do with date created)
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(arg_list, "vt:p:")
    ...
    #TO DO - add in option to get just a instrument!
    ...
    for opt,argu in opts:
        ...
        if "-v" in opt:
            ...
            test_run = argu
            print('will run test for %s new instruments'% test_run)
        elif "-p" in opt:
            start_dir = argu
    ...
    print(start_dir, os.path.exists(start_dir))
    if start_dir and os.path.exists(start_dir):
        main(start_dir,verbose)
